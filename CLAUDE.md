...
| `support/archivio.ts` | Direct primitives on the `securities` price cache (`seminaTitolo`, `rimuoviTitolo`, `ripristinaTitoli`) and on the `price_observations` history (`seminaOsservazioni`, `rimuoviOsservazioni`, `leggiOsservazioni`, `ripristinaOsservazioni`). Test-only, never imported by the server. |
...
- **One seeded ISIN per spec file — never share a key across files.** Playwright runs *files* in parallel across workers (`fullyParallel: false` only serialises within a file), and seed-then-restore is an undo stack: if two files seed the same ISIN and interleave, the last one to restore rolls back to the *other* file's seed rather than to the original, leaving residue no conditional check can undo. `e2e/support/titoli.ts` therefore assigns each file its own constant. Seeding the same ISIN repeatedly *inside* one file is safe.
  ...
  Since US-040 this is **a check, not an exhortation**. Every constant in `titoli.ts` carries a `file` field naming the `*.spec.ts` that owns it — the only one allowed to seed or remove it — and an optional `lettoDa` listing files that may *name* the key without ever writing to the archive (serving its fields with `route.fulfill()`, typing it into an intercepted search). `e2e/support/verifica-chiavi.ts` derives the registry from the module's exports, so adding a constant registers it and forgetting to register one is impossible by construction. Rules R1–R5 are documented in that file's `trovaViolazioni`; the header of `titoli.ts` explains how to declare a key, when `lettoDa` applies, and what to do for each rule.

  The check runs in two places: `npm run check:chiavi`, part of `npm run check`, and again inside `globalSetup`, so a local `npx playwright test` fails *before* the tests rather than intermittently inside them. Its pure core is unit-tested by `e2e/support/verifica-chiavi.test.ts`, run by `npm run test:e2e-support` (Vitest, `e2e/vitest.config.ts`) — hence the suffix split: `*.spec.ts` is Playwright, `*.test.ts` is Vitest, and `playwright.config.ts` narrows `testMatch` accordingly.

  Note the corollary for the borrowing case: a file that merely *enrols positions* on an ISIN is not a reader. The summary view resolves the price with a LEFT JOIN, so the premise exists and must be built — its own key, seeded by the file. US-011, US-012, US-013, US-017, US-026 and US-031 used to inherit `IE00B4L5Y983` from US-025 and passed only because that row happened to be in the dev archive.
